<?php
  ...
  $sample = $_SESSION['type'];

  $query = mysqli_query($connect, "SELECT * FROM user WHERE user_type = '".$sample."' ") or die ("ERROR: ".mysqli_error($connect));
    ...
    <div estyle=" text-align: right;">
      ...
      <div id="add" class="add_modal text-center">
        ...
        <div class=" add_container">
          <form class="add_modal-content animate" action="add_user.php" method="POST" enctype="multipart/form-data">
...
<?php
      $fetch_user = mysqli_query($connect, "SELECT * FROM user")
      ...
      while($row = mysqli_fetch_array($fetch_user)){
        ...
        echo'<tr>
        <td>'.$row['user_id'].'</td>
        <td><img class="avatar img-circle" width="55" height="55" src="data:image/jpeg;base64,  '.base64_encode($row['image']).'" accept="image/* alt="Item"></td>
        <td>'.$row['lastname'].'</td>
        <td>'.$row['firstname'].'</td>
        <td>'.$row['email'].'</td>
        <td>•••••••••••</td>
        ...
        <div class="text-center">
          <img id="output2" src="../images/$row["profile_pic"]" class="avatar img-circle" accept="image/*" alt="avatar" height="100" width="100">
          <h6>Upload a different photo...</h6>
          ...
          <input class="form-control" type="file" name="profile_pic" onchange="loadFile2(event)&validateFileType()">
           ...
           <button type="submit" class="glyphicon glyphicon-remove" onclick="window.location.href="admin_user.php" id="cancel"> Cancel</button>
      </form>
              </div>
            </div>
