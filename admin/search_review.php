...
include_once('../connect/connect.php');
	...
	$review_query = "SELECT * FROM reviews, hotels, user WHERE (review_date like '%".$search."%') OR (review_content like '%".$search."%') OR (review_title like '%".$search."%') OR (rating like '%".$search."%') OR (hotel_name like '%".$search."%') OR (email like '%".$search."%') AND hotels.hotel_id = reviews.hotel_id AND user.user_id = reviews.user_id AND reviews.user_id = user.user_id GROUP BY review_id";
	$query = mysqli_query($connect, $review_query) or die ("ERROR: " .mysqli_error($connect));
?>
...
<?php
  ...
  <div class="" method = "GET" >
    ...
    <tbody>';
        ...
        <td><a class="btn btn-danger" name="delete" onclick="javascript:confirmationDelete(this);return false;" href="deletereview.php?review_id='.$review_id.'">Delete Record</a></td>
        </tr>';
      }
  }
    else
      echo '<div style="opacity: 0.5; font-family: sanserif;">
            <br><br><h2 style="text-indent: 250px;">No Result Found...</h1>
